...
public class LocalDataSource implements IDataSource {
    ...
    @Override
    public void queryTaskListByUser(DatabaseReference myRef, final String userId, final DbCallback dbCallback) {
        // Read from the database
        myRef.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                // This method is called once with the initial value and again
                // whenever data at this location is updated.
                dbCallback.onSuccess(FbHelper.getInstance().getTaskTeamByName(dataSnapshot, userId));
            }
